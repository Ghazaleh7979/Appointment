...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user, tokens = login_user_service(
            ...
            create_audit_log(
                ...
                ip_address=get_client_ip(request),
                user_agent=get_user_agent(request),
                metadata={"phone_number": serializer.validated_data["phone_number"]},
            )
            return Response(
                {"detail": str(e)},
...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        ...
        user = request_password_reset_service(
            phone_number=serializer.validated_data["phone_number"]
        )
        
        create_audit_log(
            user=user,
            event_type=AuditLog.EventType.PASSWORD_RESET_REQUEST,
            ip_address=get_client_ip(request),
            user_agent=get_user_agent(request),
        )   
        return Response(
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user = confirm_password_reset_service(
                token=serializer.validated_data["token"],
                new_password=serializer.validated_data["new_password"],
            )
            create_audit_log(
                user=user,
                event_type=AuditLog.EventType.PASSWORD_RESET_SUCCESS,
                ip_address=get_client_ip(request),
                user_agent=get_user_agent(request),
            )   
        except DjangoValidationError as e:
